...
class dnf:
    def __init__(self, configFileName):
        ...
        self.axonalVel = config.getfloat('Model', 'axonalVelocity')
    ...
    def printParams(self):
        ...
        print "Axonal transmission velocity: {}".format(self.axonalVel)
    ...
    def initial_conditions(self, time):
        ...
        self.X1 = np.random.uniform(0.01, 0.05, (time, n))
        self.X1[:, n//4:] = 0.0

        self.X2 = np.random.uniform(0.01, 0.05, (time, n))
        self.X2[:, :3*n//4] = 0.0
    ...
    def run(self, tf, dt):
        ...
        delays = np.floor(delays/self.axonalVel)
        maxDelay = int(delays[0].max())

        # Set the initial conditions and the history
        self.initial_conditions(simTime)

        # Initialize the cortical and striatal inputs
        Cx = np.random.normal(0, 0.05, (simTime, m))  # * 0.027
        Str = np.random.normal(0, 0.03, (simTime, m))  # * 0.009
        ...
        for i in range(maxDelay, simTime):
            if i*dt > 500:
                U_[i] = self.Kc * (self.X1[i-1, :m] - Xref)
                U = self.switch * A[:m] * U_[i]
            ...
            self.X1[i, :m] = (self.X1[i-1, :m] + (-self.X1[i-1, :m] +
                              self.G1(-pre12 + Cx[i-1] - U)) * dt/self.tau1)
            self.X2[i, k:] = (self.X2[i-1, k:] + (-self.X2[i-1, k:] +
                              self.G2(pre21 - pre22 - Str[i-1]))*dt/self.tau2)
        t1 = tm.time()
